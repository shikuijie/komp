...
export function getHead (h, wrappers, row) {
  return h('thead', wrappers.map((rowCells, i) => h('tr', rowCells.reduce((res, wrapper) => {
    ...
    var attrs = getAttrs(node)
    if (attrs.rowspan !== 1) {
      spans.rowspan = attrs.rowspan
    }
    if (attrs.colspan !== 1) {
      spans.colspan = attrs.colspan
    }
    delete attrs.rowspan
    delete attrs.colspan

    // thead 中的 TCell 其实是一个 div，所以在其外面再包一层 th
    res.push(h('th', {
      attrs: spans
    }, [node]))
...
export function getWrappers (slots) {
  slots = slots || []
  var expanded = expandCells(slots.filter(s => s.tag))
...
// 清理rowspan和colspan属性
export function clearSpan (rows) {
  rows.forEach(row => {
    row.forEach(node => {
      var attrs = getAttrs(node)
      delete attrs.colspan
...
*/
export function adjustSpan (wrappers) {
...
*        如果上下相邻元素相同，则表示要行合并
*/
export function expandCells (forest) {
  if (Array.isArray(forest)) {
    return forest.reduce((res, f) => {
      var expanded = expandCells(f)
      if (!expanded) {
        return res
      }
      if (!res) {
  ...
  if (!children || forest.tag !== children[0].tag) {
    ...
    // 子元素是嵌套的tcell
    let expanded = expandCells(children.filter(child => child.tag))
    if (expanded) {
...
export function parseMRow (row, keys) {
  fixRow(row, keys)

<template>
<div class="km-tip-body" @click.stop
    :class="{km_tip_visible: visible}" :style="tipStyle">
  <div class="km_tip_triangle" :style="triangleStyle" :class="`km_tip_${direction}`">
...
export default {
  ...
  created () {
    ...
    this.bus.$on('tip.hide', () => {
      var ctx = this.eid.mCtx
      this.visible = false
      this.eid = null
      this.edata = null

      if (ctx && ctx.$el && ctx.trigger === 'click') {
        Tip.bus = null
        Tip.eid = null
      }
    })
